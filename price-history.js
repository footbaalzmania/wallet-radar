...
const AMAZON_PRODUCTS = {
  "trezor-safe-3": {
    name: "Trezor Safe 3",
    searchUrl: "https://www.amazon.de/s?k=Trezor+Safe+3",
  },
  "trezor-safe-5": {
    name: "Trezor Safe 5",
    searchUrl: "https://www.amazon.de/s?k=Trezor+Safe+5",
  },
  "trezor-safe-7": {
    name: "Trezor Safe 7",
    searchUrl: "https://www.amazon.de/s?k=Trezor+Safe+7",
  },
};
...
async function fetchEurCzkRate() {
  try {
    ...
    if (!Number.isFinite(rate) || rate <= 0) {
      throw new Error("EUR/CZK rate not found in ECB response");
    }

    return rate;
  } catch (err) {
    console.error("FX: EUR/CZK update failed:", err.message);
    return null;
  }
}
...
function convertToCzk(price, currency, eurCzkRate) {
  ...
  if (code === "EUR" && Number.isFinite(eurCzkRate)) {
    return Math.round(value * eurCzkRate * 100) / 100;
  }
...
async function updateAmazonOffers(products, eurCzkRate) {
  ...
  for (const [slug, config] of Object.entries(AMAZON_PRODUCTS)) {
    ...
    try {
      ...
      if (!Number.isFinite(price)) {
        console.log(`Amazon.de: ${config.name} price not found`);
        continue;
      }
      ...
      let offer = product.offers.find(
        (item) => String(item.store || "").toLowerCase() === "amazon.de"
      );
      ...
      if (!offer) {
        ...
        if (offer.url !== config.searchUrl) {
          offer.url = config.searchUrl;
          changed = true;
        }
      }
      ...
      console.log(
        `Amazon.de: ${config.name} = ${price.toFixed(2)} EUR` +
          (Number.isFinite(czk) ? ` (~${Math.round(czk)} CZK)` : "")
      );
    } catch (err) {
      console.error(`Amazon.de: ${config.name} update failed:`, err.message);
    }
  }
...
function collectPriceHistory(products, eurCzkRate) {
  ...
  for (const product of products) {
    ...
    for (const offer of product.offers) {
      ...
      if (
        sameStoreToday &&
        Number(sameStoreToday.price) === price &&
        String(sameStoreToday.currency || currency).toUpperCase() === currency
      ) {
        if (Number.isFinite(priceCzk) && sameStoreToday.priceCzk !== priceCzk) {
          sameStoreToday.priceCzk = priceCzk;
          sameStoreToday.exchangeRate = currency === "EUR" ? eurCzkRate : null;
          changed = true;
        }
        continue;
      }
      ...
      product.priceHistory.push({
        ...
        exchangeRate: currency === "EUR" ? eurCzkRate : null,
        store,
...
function normalizeLegacyHistory(products) {
  ...
  for (const product of products) {
    ...
    for (const entry of product.priceHistory) {
      ...
      if (!entry.priceCzk && entry.currency === "CZK") {
        entry.priceCzk = Number(entry.price);
        changed = true;
      }

      if (!entry.timestamp && entry.date) {
        entry.timestamp = `${entry.date}T00:00:00.000Z`;
        changed = true;
      }
    }
  }
...
async function runCollector() {
  ...
  let changed = normalizeLegacyHistory(products);
  ...
  if (changed && saveProducts(products)) {
    console.log(
      `Price history: saved ${today()}` +
        (Number.isFinite(eurCzkRate) ? ` | EUR/CZK ${eurCzkRate}` : "")
    );
  } else if (!changed) {
