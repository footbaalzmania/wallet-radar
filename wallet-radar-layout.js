...
<style id="wallet-radar-layout">
  ...
  @media (max-width: 640px) {
    .wallets-deal-radar {
      ...
      // Deal Radar is an EUR-only display. Apply this as the final response
      // transformation so currency labels generated by any upstream renderer
      // cannot leak CZK into the moved Deal Radar card.
      const radarStart = html.indexOf('<div class="hero-card wallets-deal-radar">');
      if (radarStart >= 0) {
        const radar = extractElement(html, radarStart);
        if (radar) {
          const radarHtml = radar.html.replace(/\bCZK\b/g, "€");
          html = html.slice(0, radarStart) + radarHtml + html.slice(radar.end);
        }
      }
