...
/*
  WalletRadar layout/display layer.
  Moves the Deal Radar card below the Trezor wallet offer grid.

  Currency rule:
  The public website displays tracked Trezor prices in EUR.
  This final response layer therefore removes any remaining CZK
  currency labels from rendered HTML after every upstream renderer
  has finished. This is intentionally global because Deal Radar and
  price/history components can be produced by different renderers.
*/
...
<style id="wallet-radar-layout">
  ...
  @media (max-width: 640px) {
    .wallets-deal-radar {
      ...
      /*
        FINAL EUR DISPLAY GUARD
        -----------------------
        All price renderers should already return EUR, but some legacy
        Deal Radar markup can still contain a literal CZK label. Perform
        the currency-label normalization only after the complete page is
        rendered, so no upstream renderer can reintroduce CZK afterward.
      */
      html = html.replace(/\bCZK\b/g, "€");
