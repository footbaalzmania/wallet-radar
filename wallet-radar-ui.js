...
function replaceWalletRenderer(source) {
  ...
  const money = (value, currency) => Number.isFinite(Number(value)) ? formatPrice(Number(value), currency || 'EUR') : '—';
  const image = product.image ? \`<a href="/go/\${escapeHtml(product.slug)}" class="wallet-image" aria-label="Buy \${escapeHtml(product.name)}"><img src="\${escapeHtml(product.image)}" alt="\${escapeHtml(product.name)}" loading="lazy"></a>\` : '';
