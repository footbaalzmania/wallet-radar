...
function removeTagById(html, tag, id) {
  ...
  return end === -1 ? html : html.slice(0,start) + html.slice(end + endMarker.length);
}
  ...
  function end(chunk, encoding, callback) {
    if (chunk) {
      ...
      if (htmlResponse) {
        ...
        html=removeTagById(html,'style','wallet-radar-ux');
        html=removeTagById(html,'script','wallet-radar-ux-script');
